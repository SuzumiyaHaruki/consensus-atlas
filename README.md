...
当前阶段已冻结 **M5.21d3 Opt-in Durable-call Runner**。通用 Campaign store 现在使用
...
M5.21d3 将增加独立显式 strategy，并按 attempt 强制 `exact intent durable -> key read -> one Step -> key clear`；
测试仍只使用注入的离线 key reader/transport，不读取桌面密钥或调用真实模型。
