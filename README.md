...
当前阶段已冻结 **M5.21d2 Offline Durable Model Planner**。通用 Campaign store 现在使用
...
显式冻结 `none/zero-model/durable-call` planner mode。d2 将只用注入的离线 transport 把该状态机
接入 etcd/raft provider，验证结果恢复和模型成本单次计费；CLI、key 和真实 HTTP 均不进入本阶段，
当前 model calls/tokens 仍为 0。
...
阅读入口： [当前阶段](docs/CURRENT_STAGE.md)、[M5.21d2 offline durable planner](docs/stage-m5.21d2-etcdraft-offline-model-planner.md)、[M5.21d1 durable model call](docs/stage-m5.21d1-durable-model-call.md)、
