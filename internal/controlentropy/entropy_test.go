...
func TestReplayValidatesEveryRequest(t *testing.T) {
	fresh := mustProvider(t, []byte("seed"))
	domain := nodeDomain("n1", 1)
	want, err := fresh.Intn(domain, 17)
	if err != nil {
		t.Fatal(err)
	}
	bytesWant, err := fresh.Bytes(domain, 12)
	if err != nil {
		t.Fatal(err)
	}
	tape, err := fresh.Tape()
	...
	replay, err := controlentropy.NewReplay([]byte("seed"), tape)
	if err != nil {
		t.Fatal(err)
	}
	got, err := replay.Intn(domain, 17)
	if err != nil {
		t.Fatal(err)
	}
	bytesGot, err := replay.Bytes(domain, 12)
	if err != nil {
		t.Fatal(err)
	}
	if got != want || !bytes.Equal(bytesGot, bytesWant) {
		t.Fatal("replay result differs")
	}
	if err := replay.CompleteReplay(); err != nil {
		t.Fatal(err)
	}
}

func TestReplayRejectsChangedBound(t *testing.T) {
	fresh := mustProvider(t, []byte("seed"))
	domain := nodeDomain("n1", 1)
	if _, err := fresh.Intn(domain, 17); err != nil {
		t.Fatal(err)
	}
	tape, err := fresh.Tape()
	if err != nil {
	...
	if _, err := replay.Intn(domain, 18); !controlentropy.IsReplayDivergence(err) {
		t.Fatalf("Intn() error = %v, want replay divergence", err)
	}
}

func TestReplayRejectsMissingDraw(t *testing.T) {
	fresh := mustProvider(t, []byte("seed"))
	if _, err := fresh.Intn(nodeDomain("n1", 1), 10); err != nil {
		t.Fatal(err)
	}
	tape, err := fresh.Tape()
	if err != nil {
	...
	if err := replay.CompleteReplay(); !controlentropy.IsReplayDivergence(err) {
		t.Fatalf("CompleteReplay() error = %v, want replay divergence", err)
	}
}
