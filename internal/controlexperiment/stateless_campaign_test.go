...
func TestStatelessCampaignPersistsAgentFailureWithoutInventingTokenUsage(t *testing.T) {
	agent, err := NewStatelessAgentTraversalMethod(
		"fixture-agent", "fixture-planner", fixtureStatelessAgentKnowledge(t),
	)
	if err != nil {
		t.Fatal(err)
	}
	discovery := fixtureStatelessCampaignDiscovery(t, agent)
	spec, err := NewStatelessCampaignSpec(StatelessCampaignSpec{
		ID: "fixture-agent-spec", TargetID: "fixture-target",
		TargetIdentityDigest: strings.Repeat("a", 64), Methods: []StatelessTraversalMethod{agent},
		CorpusDigest: discovery.CorpusDigest, SourceBundleDigest: strings.Repeat("b", 64),
		SourceManifestDigest: strings.Repeat("c", 64), SourceWork: campaignTestWork(2, 2, 0, 0),
		RootCount: 1, MaxDepth: 1, MaxWorkItemsPerRoot: 1, MaxSearchWorkUnitsPerRoot: 10,
		Budget: StatelessCampaignAttemptBudget{
	...
	config, err := NewStatelessCampaignConfig("fixture-agent-failure", spec, 1_000)
	if err != nil {
		t.Fatal(err)
	}
	head, err := NewCampaignCheckpoint(config)
	...
	callRequest := fixtureStatelessAgentCallRequest(t)
	...
	intent, err := NewStatelessAgentCallIntent(
		"fixture-campaign-call", 1, "fixture-root", callRequest, transport,
	...
	result, err := NewStatelessAgentCallResult(intent, dispatch, StatelessAgentCallResult{
		Status: StatelessAgentCallFailed, FailureCode: "agent-transport-failed",
		Work: ModelWork{Calls: 1},
	})
	if err != nil {
		t.Fatal(err)
	}
	audit, err := NewStatelessAgentCallAudit(intent, &dispatch, &result)
	if err != nil {
		t.Fatal(err)
	}
	artifact, err := NewStatelessCampaignAttemptArtifact(request, spec, StatelessCampaignExecution{
		ModelWork: ModelWork{Calls: 1}, AgentCalls: []StatelessAgentCallAudit{audit},
		Failure: &MethodFailure{
			Phase: "agent-call", Code: "AGENT_TRANSPORT_FAILED",
		},
	})
	if err != nil || artifact.Outcome != CampaignAttemptFailed ||
		artifact.Work.Model != (ModelWork{Calls: 1}) {
		t.Fatalf("dispatched Agent failure was not durably chargeable: %#v/%v", artifact, err)
	}
}
