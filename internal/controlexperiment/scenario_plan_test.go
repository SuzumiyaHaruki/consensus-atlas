...
func TestScenarioMilestoneWaitUsesAuthoritativeTargetClosureStop(t *testing.T) {
	ctx := context.Background()
	runtimeConfig := RuntimeConfig{SeedHex: "6d346c342d636c6f737572652d77616974", MaxClones: 1}
	root := fixtureInitialTrace(t, ctx, runtimeConfig)
	spec, err := semantic.NewRiskWitnessSpec(
		"fixture-closure-wait-risk", "fixture-cft", "closure-wait",
		[]string{"preferred-prefix"}, nil,
	)
	if err != nil {
		t.Fatal(err)
	...
	projector := fixtureSemanticPrefixProjector{preferred: "never-selected"}
	rootRisk, err := projector.Project("fixture-closure-wait-root", spec, root)
	if err != nil {
		t.Fatal(err)
	}
	for _, test := range []struct {
		...
		{name: "underdetermined", status: ScenarioClosureUnderdetermined,
			stop:       ScenarioProgressClosureUnderdetermined,
			reasonCode: ScenarioReasonMilestoneWaitClosureUnderdetermined},
		{name: "no-eligible", status: ScenarioClosureNoEligible,
			stop:       ScenarioProgressClosureQuiescent,
			reasonCode: ScenarioReasonMilestoneWaitClosureQuiescent},
	} {
		t.Run(test.name, func(t *testing.T) {
			factoryCalls := 0
			result, runErr := ExecuteSemanticBoundedScenarioPlan(
				ctx, "fixture-closure-wait-"+test.name,
				ScenarioPlan{ID: "closure-wait-" + test.name, Steps: []ScenarioStep{
					{ID: "crash-n1", Selector: FrontierActionSelector{Kind: control.ActionCrash, Node: "n1"}},
					{ID: "wait", AfterMilestone: "preferred-prefix",
						Selector: FrontierActionSelector{Kind: control.ActionRestart, Node: "n1"}},
				}},
				2, 4, spec, rootRisk, root, runtimeConfig,
				&FaultEnvelope{MaxCrashes: 1, MaxConcurrentCrashes: 1},
				func() (control.Adapter, error) { return fixture.New(), nil }, projector,
				func(_ controlruntime.Trace, next RiskFrontierView, _ controlruntime.Snapshot) (
					ScenarioSemanticExposure, error,
				) {
					return unknownScenarioSemantics(t, next), nil
				},
				func(context ScenarioClosureContext) (ScenarioClosureSelector, bool, error) {
					factoryCalls++
					if context.Intervention.Action.Kind != control.ActionCrash ||
						context.Intervention.Action.Node.Node != "n1" ||
						len(context.Trace.Records) != len(root.Records)+1 {
						t.Fatalf("closure did not receive the executed intervention: %#v", context)
					}
					return func(ActionFrontierView) (ScenarioClosureSelection, error) {
						return ScenarioClosureSelection{Status: test.status}, nil
					}, true, nil
				},
				0,
			)
			if runErr != nil || factoryCalls != 1 || result.Status != ScenarioStatusStopped ||
				len(result.Steps) != 2 || len(result.AutomaticProgress) != 0 ||
				len(result.FinalTrace.Records) != len(root.Records)+1 ||
				result.NaturalProgressStop != test.stop ||
				result.Steps[1].ReasonCode != test.reasonCode ||
				len(result.Steps[1].Available) == 0 {
				t.Fatalf("Target closure stop fell back to public order: %#v/%v", result, runErr)
			}
		})
	}
	budgetResult, err := ExecuteSemanticBoundedScenarioPlan(
		ctx, "fixture-closure-wait-budget",
		ScenarioPlan{ID: "closure-wait-budget", Steps: []ScenarioStep{
			{ID: "crash-n1", Selector: FrontierActionSelector{Kind: control.ActionCrash, Node: "n1"}},
			{ID: "wait", AfterMilestone: "preferred-prefix",
				Selector: FrontierActionSelector{Kind: control.ActionRestart, Node: "n1"}},
		}},
		2, 1, spec, rootRisk, root, runtimeConfig,
		&FaultEnvelope{MaxCrashes: 1, MaxConcurrentCrashes: 1},
		func() (control.Adapter, error) { return fixture.New(), nil }, projector,
		func(_ controlruntime.Trace, next RiskFrontierView, _ controlruntime.Snapshot) (
			ScenarioSemanticExposure, error,
		...
		func(ScenarioClosureContext) (ScenarioClosureSelector, bool, error) {
			return func(frontier ActionFrontierView) (ScenarioClosureSelection, error) {
				for _, action := range frontier.Actions {
					if scenarioClosureActionKind(action.Kind) {
						return ScenarioClosureSelection{Status: ScenarioClosureSelected, Action: action}, nil
					}
				}
				return ScenarioClosureSelection{Status: ScenarioClosureNoEligible}, nil
			}, true, nil
		},
		0,
	)
	if err != nil || budgetResult.NaturalProgressStop != ScenarioProgressClosureBudget ||
		len(budgetResult.Steps) != 2 ||
		budgetResult.Steps[1].ReasonCode != ScenarioProgressClosureBudget {
		t.Fatalf("closure budget reason was hidden by generic budget feedback: %#v/%v",
			budgetResult, err)
	}
}

func TestScenarioMilestoneAlreadyReachedDoesNotBuildClosure(t *testing.T) {
	ctx := context.Background()
	runtimeConfig := RuntimeConfig{SeedHex: "6d346c352d6c617a792d636c6f73757265", MaxClones: 1}
	root := fixtureInitialTrace(t, ctx, runtimeConfig)
	spec, err := semantic.NewRiskWitnessSpec(
		"fixture-lazy-closure-risk", "fixture-cft", "lazy-closure",
		[]string{"crash-prefix"}, nil,
	)
	if err != nil {
		t.Fatal(err)
	...
	projector := actionKindSemanticProjector{}
	rootRisk, err := projector.Project("fixture-lazy-closure-root", spec, root)
	if err != nil {
		t.Fatal(err)
	}
	factoryCalls := 0
	result, err := ExecuteSemanticBoundedScenarioPlan(
		ctx, "fixture-lazy-closure",
		ScenarioPlan{ID: "lazy-closure", Steps: []ScenarioStep{
			{ID: "crash-n1", Selector: FrontierActionSelector{Kind: control.ActionCrash, Node: "n1"}},
			{ID: "restart-n1", AfterMilestone: "crash-prefix",
				Selector: FrontierActionSelector{Kind: control.ActionRestart, Node: "n1"}},
		}},
		2, 2, spec, rootRisk, root, runtimeConfig,
		&FaultEnvelope{MaxCrashes: 1, MaxConcurrentCrashes: 1},
		func() (control.Adapter, error) { return fixture.New(), nil }, projector,
		func(_ controlruntime.Trace, next RiskFrontierView, _ controlruntime.Snapshot) (
			ScenarioSemanticExposure, error,
		...
		func(ScenarioClosureContext) (ScenarioClosureSelector, bool, error) {
			factoryCalls++
			return nil, false, errors.New("closure should not be built")
		},
		0,
	)
	if err != nil || factoryCalls != 0 || result.Status != ScenarioStatusCompleted ||
		len(result.Steps) != 2 || result.FinalRisk.Status != semantic.RiskWitnessReached {
		t.Fatalf("already reached milestone unnecessarily built closure: %#v calls=%d err=%v",
			result, factoryCalls, err)
	}
}
...
func TestScenarioAgentClosureOwnsRemainingGlobalDecisionBudget(t *testing.T) {
	...
	envelope := &FaultEnvelope{MaxCrashes: 1, MaxConcurrentCrashes: 1}
	spec, err := semantic.NewRiskWitnessSpec(
		"fixture-agent-closure-risk", "fixture-cft", "agent-closure-feedback",
		[]string{"preferred-prefix"}, nil,
	)
	if err != nil {
		t.Fatal(err)
	...
	projector := fixtureSemanticPrefixProjector{preferred: "never-selected"}
	rootRisk, err := projector.Project("fixture-agent-closure-root", spec, root)
	if err != nil {
		t.Fatal(err)
	}
	factory := func() (control.Adapter, error) { return fixture.New(), nil }
	frontier, _, _, err := ReconstructRiskFrontierState(
		ctx, "fixture-agent-closure-frontier", spec, rootRisk, root, len(root.Records),
		runtimeConfig, envelope, factory,
	)
	if err != nil {
		t.Fatal(err)
	...
	knowledge, err := NewProtocolKnowledgePack(ProtocolKnowledgePack{
		ID: "fixture-agent-closure-knowledge", Family: spec.FamilyID, Protocol: "fixture-consensus",
		Knowledge: []KnowledgeStatement{{
			ID: "closure-feedback", Text: "A Target closure stop must be returned for plan revision.",
		}},
		Risks: []ProtocolRisk{{
			ID: spec.RiskID, Summary: "Stop an ambiguous post-intervention closure.",
			RequiredCapabilities: []string{"natural-time"},
			RequiredActions:      []control.ActionKind{control.ActionCrash},
			AllowedBackendIDs:    []string{ScenarioPlanningBackendID},
		}},
	...
	hypothesis, err := NewTestHypothesis(
		"fixture-agent-closure-hypothesis", knowledge, spec,
		"Use trusted closure feedback to revise the investigation.", ScenarioPlanningBackendID,
	)
	if err != nil {
		t.Fatal(err)
	}
	calls, closureCalls := 0, 0
	result, err := ExploreScenarioWithPlannerAndClosure(
		ctx, 3, 1, 6, knowledge, hypothesis, spec, frontier,
		unknownScenarioSemantics(t, frontier), rootRisk, root, runtimeConfig, envelope,
		nil, nil, factory, projector,
		func(_ controlruntime.Trace, next RiskFrontierView, _ controlruntime.Snapshot) (
			ScenarioSemanticExposure, error,
		...
		func(context ScenarioClosureContext) (ScenarioClosureSelector, bool, error) {
			closureCalls++
			if context.Intervention.Action.Kind != control.ActionCrash {
				t.Fatalf("unexpected closure intervention: %#v", context.Intervention)
			}
			if closureCalls == 1 {
				return func(ActionFrontierView) (ScenarioClosureSelection, error) {
					return ScenarioClosureSelection{Status: ScenarioClosureUnderdetermined}, nil
				}, true, nil
			}
			return func(frontier ActionFrontierView) (ScenarioClosureSelection, error) {
				for _, action := range frontier.Actions {
					if scenarioClosureActionKind(action.Kind) {
						return ScenarioClosureSelection{
							Status: ScenarioClosureSelected, Action: action,
						}, nil
					}
				}
				return ScenarioClosureSelection{Status: ScenarioClosureNoEligible}, nil
		...
		func(_ context.Context, view ScenarioAgentView) ([]byte, ModelWork, error) {
			...
			if calls == 2 {
				if view.Prior == nil ||
					view.Prior.ReasonCode != ScenarioProgressClosureUnderdetermined ||
					view.Prior.NaturalProgressStop != ScenarioProgressClosureUnderdetermined ||
					view.Prior.ProgressDelta == nil ||
					view.Prior.ProgressDelta.NaturalProgressStop != ScenarioProgressClosureUnderdetermined {
					t.Fatalf("Agent did not receive structured closure feedback: %#v", view.Prior)
				}
				var temporal FrontierActionRef
				for _, action := range view.Frontier.Actions {
					if action.Kind == control.ActionFireTemporal {
				...
				if temporal.ActionID == "" {
					t.Fatalf("revised path has no non-intervention Action: %#v", view.Frontier)
				}
				encoded, marshalErr := json.Marshal(ScenarioInvestigationProposal{
					Intent: ScenarioIntentRevise,
					Plan: ScenarioPlan{ID: "agent-closure-revise", Steps: []ScenarioStep{{
						ID: "advance-time", Selector: FrontierActionSelector{ActionID: temporal.ActionID},
					}}},
				})
				return encoded, ModelWork{}, marshalErr
			}
			...
			encoded, marshalErr := json.Marshal(ScenarioInvestigationProposal{
				Intent: ScenarioIntentContinue,
				Plan: ScenarioPlan{ID: "agent-closure-crash", Steps: []ScenarioStep{{
					ID: "crash-n1", Selector: FrontierActionSelector{
						Kind: control.ActionCrash, Node: "n1",
					},
				}}},
			})
			return encoded, ModelWork{}, marshalErr
