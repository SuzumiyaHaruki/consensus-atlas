...
// an existing trusted Risk milestone. The plan carries no execution budget,
// fault policy, assertion, verdict, or trusted digest.
...
// final Trace remains the sole exact execution record and every applied or
// automatic choice is materialized and fresh-replayed by the same substrate.
...
func ExecuteBoundedScenarioPlan(
	...
	return executeBoundedScenarioPlan(
		...
		runtimeConfig, faultEnvelope, newAdapter, projector, nil,
...
func ExecuteSemanticBoundedScenarioPlan(
	...
	return executeSemanticBoundedScenarioPlan(
		ctx, executionID, plan, maxSteps, maxDecisions, spec, rootRisk, root,
	...
	for index, step := range plan.Steps {
		...
		if step.AfterMilestone != "" {
			...
			for !scenarioRiskHasMilestone(result.FinalRisk, step.AfterMilestone) {
				...
				action, ok := scenarioNaturalProgressActionWithFocus(view.Actions, progressFocus)
				if !ok {
