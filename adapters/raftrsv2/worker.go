...
func (client *workerClient) call(request workerRequest) (workerResponse, error) {
	if client == nil || client.closed {
		return workerResponse{}, errors.New("RAFT_RS_WORKER_CLOSED")
	}
	client.nextID++
	request.ID = client.nextID
	if err := client.encode.Encode(request); err != nil {
		return workerResponse{}, fmt.Errorf("RAFT_RS_WORKER_WRITE: %w", err)
	}
	var response workerResponse
	if err := client.decode.Decode(&response); err != nil {
		return workerResponse{}, fmt.Errorf("RAFT_RS_WORKER_READ: %w: %s", err, client.stderr.String())
	}
	if response.SchemaVersion != workerSchema || response.ID != request.ID {
		return workerResponse{}, errors.New("RAFT_RS_WORKER_RESPONSE_IDENTITY_MISMATCH")
	}
	if !response.OK {
		return workerResponse{}, fmt.Errorf("RAFT_RS_WORKER_REJECTED: %s", response.Error)
	}
	return response, nil
}
...
func (client *workerClient) close() error {
	...
	if waitErr != nil {
		return fmt.Errorf("RAFT_RS_WORKER_EXIT: %w: %s", waitErr, client.stderr.String())
	}
	return nil
}
