...
// Strict decoding therefore rejects even an empty pre-seeded hypothesis
// instead of treating it as equivalent to an absent field.
...
func (config etcdraftAgentExperimentConfig) validate() error {
	if config.validateAgentic() != nil || config.SearchMaxDepth <= 0 ||
		config.SearchMaxWorkItems <= 0 || config.SearchMaxWorkUnits <= 0 ||
		config.ExplorerBudget.Validate() != nil {
		return errors.New("ETCDRAFT_AGENT_EXPERIMENT_CONFIG_INVALID")
	}
	return nil
}

// validateAgentic checks only fields consumed by agentic-episode-v1. The
// legacy Explorer/search fields remain required by validate, but a new
// Agentic input does not need to carry unused A2 configuration.
